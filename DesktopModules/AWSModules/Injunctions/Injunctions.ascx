<%@ Control Language="vb" Inherits="AWS.Modules.Injunctions.ViewInjunctions" CodeFile="Injunctions.ascx.vb" AutoEventWireup="false" Explicit="True" %>
...
<div id="InjunctionForm">
    <div style="margin-top: 15px;" id="InjunctionList">
    ...
    <asp:UpdatePanel ID="dptUpdate" runat="server" RenderMode="Block" OnUnload="UpdatePanel_Unload">
        ...
        <p class="alert alert-danger"><em class="fa fa-warning"></em> The Injunctions highlighted below have been completed and will be automatically deleted 24 hours after viewing.</p>
            </asp:Literal>
            <div class="btn-toolbar " role="toolbar" aria-label="Toolbar Group" style="display: block; margin-bottom: 8px">
                ...
                <div class="btn-group pull-right " role="group" aria-label="Action group">
                    <strong>
                        <asp:LinkButton ID="cmdShowAssigned" Visible="false" runat="server" CssClass="" CommandArgument="true"><em class="fa fa-check-square fa-lg"></em> Show only my Injunctions</asp:LinkButton></strong>
                </div>
            </div>
            ...
            <div class="form-inline" id="dvFilters" runat="server">
                ...
                <div class="form-group">
                    &nbsp;<asp:HyperLink ID="lnkRefresh" runat="server" ToolTip="Refresh Page"><em class="fa fa-refresh fa-lg"></em></asp:HyperLink>
                </div>
            </div>
            <asp:Repeater runat="server" ID="rptInjunctions">
                <HeaderTemplate>

                    <table id="Injunctions" summary="List of Injunctions" class="table table-striped">
                ...
                <ItemTemplate>
                    <tr id="tablerow" runat="server">
                        <td class="command command-icon">
                            <asp:LinkButton ID="lnkSign" runat="server" data-toggle="tooltip" data-placement="top" title="Sign Document" Visible="false" CommandName="sign" CommandArgument='<%#Eval("InjunctionId") %>'>
                                <em class="fa fa-edit">&nbsp;</em>
                            </asp:LinkButton>
                        </td>
                        <td>
                            <%#DataBinder.Eval(Container.DataItem, "InjunctionId")%>
                        </td>
                        <td>
                            <asp:HyperLink ID="lnkInjunction" CssClass="InjunctionLink" Target="_blank" runat="server"><%#Eval("title")%></asp:HyperLink>
                        </td>
                        ...
                        <td class="command command-icon">
                            <asp:LinkButton ID="cmdRelease" runat="server" CommandArgument='<%#DataBinder.Eval(Container.DataItem, "InjunctionId").ToString() %>' CommandName="Release"><em class="fa fa-lock" data-toggle="tooltip" data-placement="top" title="Release Injunction"></em></asp:LinkButton>
                            <asp:LinkButton ID="cmdDelete" runat="server" ToolTip="Delete Injunction" CommandName="Delete" OnClientClick="return confirm('Delete this Injunction?')" CommandArgument='<%#DataBinder.Eval(Container.DataItem, "InjunctionId").ToString() %>'><em class="fa fa-trash"></em></asp:LinkButton>
                        </td>
...
<script type="text/javascript">
    function pageLoad(sender, args) {
        $(document).ready(function () {
            $('#InjunctionForm label').addClass("control-label");
    ...
    function bindDataTable() {
        ...
        var Injunctions = $('#Injunctions').DataTable({
            "searching": false,
            "aoColumns": [
                { "bSortable": false },
                null,
        ...
        $('#Injunctions_wrapper label').css("font-weight:700");
