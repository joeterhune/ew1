...
#Region "Methods"
        ...
        Private Sub CancelCheckout(redirect As Boolean)
            Try
                ...
                Dim objwarrant As WarrantsInfo = _ctl.GetWarrants(_warrantId)
                If objwarrant.StatusId = WarrantStatus.UnderReview Then
                    objwarrant.StatusId = WarrantStatus.NewWarrant
                End If
                _ctl.UpdateWarrants(objwarrant)
                ...
                If Not Page.IsPostBack Then
                    ...
                    If IsNumeric(_warrantId) Then
                        ...
                        If Not objwarrant Is Nothing Then
                            ...
                            Dim objwarrantFile As WarrantImage = _ctl.GetFile(objwarrant.FileId)
                            If Not objwarrantFile Is Nothing Then
                                Dim buffer As Byte() = _encryptor.DecryptStream(objwarrantFile.Bytes)
...
#Region "Remote Calls"
        ...
        <RemoteInvokable>
        Public Function ApproveWarrant(enc As String) As String
            ...
            Try
                ...
                If UserInfo.Username = "" Then
                    ...
                    Return returnString
                End If
                _warrantId = _encryptor.QueryStringDecode(enc, UserInfo.Username)
                Dim objwarrant As WarrantsInfo = _ctl.GetWarrants(_warrantId)
                If Not objwarrant Is Nothing Then
                    If Not objwarrant.IsMarked Then
                    ...
                    End If
                    WebAnnotationViewer1.ClearAnnotations()
                    'If Imageprocessing.CreatePDF(GetBytsForFile(path), warrantTitle, objwarrant,_ctl) Then
                    '    _ctl.DeleteFile(objwarrant.FileId)
                    'Else
                        Dim encryptedBytes As Byte() = _encryptor.EncryptStream(GetBytsForFile(path))
                        _ctl.UpdateFile(objwarrant.FileId, encryptedBytes)
                    'End If
        ...
        <RemoteInvokable>
        Public Function CancelWarrant(enc As String) As String
            Dim returnString As String = "2|default"
            Try
                WebAnnotationViewer1.ClearAnnotations()
                If UserInfo.Username = "" Then
        ...
        <RemoteInvokable>
        Public Function RejectWarrant(enc As String, rejecttest As String) As String
            Dim returnString As String = "2|default"
            Try
                Dim frame As Integer = WebAnnotationViewer1.CurrentPage - 1
                Dim url As String = WebAnnotationViewer1.ImageUrl
                ...
                If Not objwarrant Is Nothing Then
                    ...
                    Dim warrantTitle As String = objwarrant.Title
                    objwarrant.ReviewedByJudgeUserId = UserInfo.UserID
                    objwarrant.StatusId = WarrantStatus.Rejected
                    objwarrant.ReviewedDate = Now
                    _ctl.UpdateWarrants(objwarrant)
                    If info.FrameCount = WebAnnotationViewer1.Annotations.Layers.Count Then
                        For fi As Integer = 0 To info.FrameCount - 1
...
#Region "Annotations"
        ...
        Private Function CreateStamp(location As System.Drawing.PointF, stampText As String, resolution As Single) As TextAnnotation
